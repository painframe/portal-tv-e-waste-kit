...
3. **Audio capture ambiguity**: HDMI capture may or may not include the Portal's mic audio. Per [S5], the mic is gated separately (`com.facebook.secure` permission for the mic array). HDMI-embedded audio is the *display*'s audio output, not the mic. **Use a separate USB or Bluetooth mic.** Same caveat as `portal-tv-webcam`.
